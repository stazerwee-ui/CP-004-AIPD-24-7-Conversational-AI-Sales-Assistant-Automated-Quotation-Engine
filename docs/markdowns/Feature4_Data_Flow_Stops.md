...
In [main.py](file:///c:/Users/staze/Desktop/AIPD/codes17aug2026/codes17aug2026/main.py#L543-L840), `extract_state_from_history()` scans the conversation and extracts canonical catalog keys, while protecting inquiries via `is_pure_inquiry`:

```python
# main.py - Pure Inquiry Guard & Keyword Extraction
is_question_or_inquiry = (
    ...
    or kw(text_lower, "vs", "versus", "difference", "compare", "comparison", "better", "which one")
)
...
is_pure_inquiry = is_question_or_inquiry and not has_explicit_selection
...
Because `turn_intent == "QUALIFIED_SELECTION"`, the engine commits `tier = "deluxe"` to active state, while leaving `deceasedName = None` intact. In contrast, for a pure question like *"What's your installment plan?"*, `is_pure_inquiry` prevents state mutation and keeps the family on the current step.
